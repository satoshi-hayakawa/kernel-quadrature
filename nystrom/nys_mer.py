...
def recombination(
    ...
    mer_kz=False,  # whether or not using the mercer decomposition of k^Z
    sq_ker=None
):
    return rc_kernel_svd(pts_rec, pts_nys, num_pts, mu=init_weights,
                         use_obj=use_obj, rand_SVD=rand_SVD, mer_kz=mer_kz, sq_ker=sq_ker)


def ker_svd_sparsify(pt, s, rand_SVD=True, H_mu=None):
    if H_mu is None:
        ...
        # H = np.linalg.cholesky(H_mu).T
        H = scipy.linalg.sqrtm(H_mu).real
        K_pinv = np.linalg.pinv(k(pt, pt), hermitian=True)
        S = H @ K_pinv @ H.T
        svd = TruncatedSVD(n_components=s) if rand_SVD else TruncatedSVD(
            n_components=s, algorithm='arpack')
        svd.fit(S)
        svs, V = svd.singular_values_, svd.components_
        U = V @ np.linalg.pinv(H.T, hermitian=True)  # using matrix sqrt
        return svs, U

# Uを書き換えればOK？
# U @ k(pt, samp) の形がテスト関数
# U.T @ diag(svs) @ U = k(pt, pt)_s (with truncation) の代わりに…
# H.T @ H = h_mu(pt, pt) <- これが引数
# H @ k(pt, pt)^+ @ H.T <- これをSVD
# ... = V.T @ eigenvalues @ V
# V.T[:s] @ (H^+).T @ k(pt, samp) <- これが新しいテスト関数
# U <- V.T[:s] @ (H^+).T とすればよい


def rc_kernel_svd(samp, pt, s, mu=0, use_obj=True, rand_SVD=True, mer_kz=False, sq_ker=None):
    # Nystrom
    ...
    if mer_kz:
        if sq_ker is None:
            ...
            H_mu /= N
            m = len(pt)
            #H_mu += 1e-10 * np.identity(m)
        else:
            H_mu = sq_ker(pt, pt)

    svs, U = ker_svd_sparsify(pt, s - 1, rand_SVD, H_mu)
