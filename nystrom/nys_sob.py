...
def contaminated(m, n):
    pts_nys = np.random.beta(a=2, b=5, size=[m, d_global])
    if d_global == 1:
        ug = np.array([[i / n] for i in range(n)])
    else:
        sampler = Halton(d=d_global)
        ug = sampler.random(n)
    pts_nys = np.concatenate((pts_nys, ug), axis=0)
    return pts_nys
...
def experiments(
    ...
    file_name = "results/d{}s{}t{}".format(dim, smooth, times)
    if pow_int != 2:
        file_name += "p{}".format(pow_int)

    text_data = open(file_name + ".txt", 'w', encoding='utf-8')
    ...
    for n in x_ex:
        ...
        for i in range(methods):
            ...
            for j in range(times):
                points, weights = func(m_names[i], n, rec=n**pow_int, nys=20*n)
...
def func(name, n, rec=0, nys=0):
    ...
    if name == '$k_s^Z$':
        ...
        pts_nys = contaminated(nys, n)
        idx, w = enys.recombination(
            pts_rec, pts_nys, n, use_obj=True, rand_SVD=False, mer_kz=True, sq_ker=functools.partial(k, s=2*s_global))
        x = pts_rec[idx]
        return x, w
